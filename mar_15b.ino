...
/*void initNewGame() {
  ...
  player.mechType = MechType::Battle_Cat;
  player.playerMechStats = playerBattleCatStats;
  player.mechStatus = MechStatus::Normal; // Initialize playerMechStats with the stats of the chosen mech
  player.health = 100;
}*/
...
void initLoadGame() {
  ...
  if (!FX::loadGameState(saveData)) {
    currentState = GameState::No_Save_Data;  // You should add this new state to your GameState enum.
  } else {
...
void updateLoadGame() {
  ...
  player.playerMechStats = saveData.saveMechStats;  // Update playerMechStats with the loaded save data
  player.health = saveData.health;
}

void updateNoSaveState() {
...
void initializeEnemiesForMission(const Mission& mission);
...
void initializeEnemiesForMission(const Mission& mission) {
  ...
  for (uint8_t i = 0; i < maxEnemies; ++i) {
    if (i < mission.numEnemies) {
      ...
      // Set other enemy properties based on mechType, e.g., health, moveSpeed
      //enemies[i].x = random(worldWidth);   // Random X position
      //enemies[i].y = random(worldHeight);  // Random Y position
    } else {
      enemies[i].state = EnemyState::Inactive;  // Set extra enemies to Inactive
    }
    ...
    switch (enemies[i].mechType) {
      ...
      case MechType::Battle_Cat:
        ...
        break;
        // Add cases for other mech types
    }
  }
}
  ...
  if (arduboy.justPressed(A_BUTTON)) {
    switch (missionMenu.currentSelection) {
      case 0:  // Deploy
        ...
        initializeEnemiesForMission(currentMission);
...
void drawEnemy(uint8_t x, uint8_t y) {
  ...
  FX::drawBitmap(x - spriteWidth / 2, y - spriteWidth / 2, sprite, frame, dbmMasked);
}
