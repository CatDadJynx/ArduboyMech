...
void handleInput() {
  ...
  if (arduboy.pressed(UP_BUTTON) || arduboy.pressed(DOWN_BUTTON)) {
    ...
    player.x += direction * player.moveSpeed * Cos(player.angle);
    player.y += direction * player.moveSpeed * Sin(player.angle); 
...
void drawEnemy(uint8_t x, uint8_t y)  //, SQ7x8 distance)
{
  ...
  // TODO: Animate sprites walking between sizes 8x11 and 70x94
  const uint24_t* sprite;
...
void drawExplosion(uint8_t x, uint8_t y) {
  ...
  if (arduboy.everyXFrames(10)) {
    if (enemies[currentEnemyIndex].currentFrame > enemies[currentEnemyIndex].maxFrames) {
      enemies[currentEnemyIndex].state = Inactive;
  ...
  for (uint8_t i = 0; i < enemyCount; ++i) {
    if (enemies[i].state != Inactive) {
      ...
      if (to3DView(enemies[currentEnemyIndex], drawParameters)) {
        ...
        if (enemies[currentEnemyIndex].state == Active) {
          drawEnemy(drawParameters.x, drawParameters.y);
        } else if (enemies[i].state == Exploding) {
...
void hitEnemy(uint8_t bulletEnemyIndex) {
  if (enemies[bulletEnemyIndex].health <= 0 && enemies[bulletEnemyIndex].state != Exploding) {
    enemies[bulletEnemyIndex].state = Exploding;
  }
...
void loop() {
  ...
  //FX::drawBitmap(0, 0, battleCatCockpit, 0, dbmMasked);
  FX::drawBitmap(0, 0, thorHammerCockpit, 0, dbmMasked);
